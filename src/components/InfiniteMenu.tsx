...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    const rawGl = this.canvas.getContext("webgl2", {
      antialias: true,
      alpha: false,
      colorSpace: "srgb",
    });
    ...
    const gl = rawGl as unknown as WebGL2RenderingContext;
    this.gl = gl;
  ...
  private render(): void {
    ...
    gl.clearColor(0.45, 0.24, 0, 1); // R, G, B, A — values from 0.0 to 1.0
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        ...
        `}
          ...
          ${
            isMoving
              ? "opacity-0 pointer-events-none duration-100 md:translate-x-[-60%] md:-translate-y-1/2"
              : "opacity-100 pointer-events-auto duration-500 md:translate-x-[-90%] md:-translate-y-1/2"
          }
        `}
          >
        ...
        `}
          >
            <p className="select-none relative text-white top-0.5 text-[26px]">&#x2197;</p>
          </div>
        </>
      )}
    </div>
